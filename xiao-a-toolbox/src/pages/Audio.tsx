...
type AudioMode = "convert" | "merge" | "trim";
const AUDIO_FORMATS = [
...
export default function AudioPage() {
  ...
  const { convertAudio, audioMerge, trimAudio, startProgress, stopProgress, cancelConvert, notify, getConfig, addHistory } = useIPC();
  ...
  const handleClick = async () => {
    const paths = await window.electronAPI.openFile([
      { name: "音频", extensions: ["mp3", "wav", "aac", "flac", "ogg", "wma", "m4a"] },
    ]);
    ...
    try {
      if (mode === "merge") {
        const savePath = outputDir
          ? `${outputDir}\\merged.${format}`
          : await window.electronAPI.saveFile({
              filters: [{ name: "音频", extensions: [format] }],
              defaultPath: "merged." + format,
            });
        ...
        ok = 1;
        addHistory({ input: files.map((f) => f.name).join(","), output: savePath, format, mode, status: "success" });
      } else {
        ...
        for (let i = 0; i < total; i++) {
          try {
            ...
            if (mode === "trim") {
              await trimAudio({ inputPath: input.path, outputPath: savePath, format, params: { start: startTime, end: endTime } });
            } else {
              await convertAudio({ inputPath: input.path, outputPath: savePath, format, params: { bitrate, sampleRate } });
            }
            ok++;
            addHistory({ input: input.path, output: savePath, format, mode, status: "success" });
          } catch {
            fail++;
            addHistory({ input: files[i].path, output: "", format, mode, status: "failed" });
          }
  ...
  return (
    <div>
      ...
      <p className="text-sm mb-5" style={{ color: "var(--text-tertiary)" }}>转换格式、合并或裁剪音频 · 支持多文件</p>

      <div className="flex gap-1 mb-5 p-1 rounded-xl" style={{ background: "var(--bg-secondary)" }}>
        {(["convert", "merge", "trim"] as AudioMode[]).map((m) => (
          <button key={m} onClick={() => setMode(m)}
            ...
            {m === "convert" ? "格式互转" : m === "merge" ? "音频合并" : "音频裁剪"}
          </button>
        ))}
      </div>
      ...
      <DropZone onFilesDrop={handleDrop}>
        <div onClick={handleClick}
          ...
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
            {mode === "merge" ? "拖拽多个音频文件（按顺序合并）" : "拖拽音频文件"}，或点击选择
          </p>
      ...
      {(mode === "convert" || mode === "merge") && (
        <div className="mt-4 rounded-xl border p-5 space-y-4" style={{ background: "var(--bg-card)", borderColor: "var(--border-subtle)", boxShadow: "var(--shadow-sm)" }}>
          ...
          {files.length > 0 && bitrate && <EstimateSize filePath={files[0].path} bitrate={bitrate} mode="audio" />}
        </div>
      )}
        ...
        <div className="mt-4 rounded-xl border p-5 space-y-4" style={{ background: "var(--bg-card)", borderColor: "var(--border-subtle)", boxShadow: "var(--shadow-sm)" }}>
          <div className="flex gap-6">
            <div>
              ...
              <input type="text" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="px-3 py-2 rounded-lg border text-sm outline-none" style={{ ...is, width: 140 }} placeholder="00:00:00" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2" style={{ color: "var(--text-secondary)" }}>结束时间</label>
              <input type="text" value={endTime} onChange={(e) => setEndTime(e.target.value)} className="px-3 py-2 rounded-lg border text-sm outline-none" style={{ ...is, width: 140 }} placeholder="00:01:00" />
            </div>
          </div>
        </div>
      )}
