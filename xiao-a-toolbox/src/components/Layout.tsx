...
const navItems = [
  ...
  { to: "/video-to-audio", label: "视频转音频", icon: "🎵" },
          ...
          <div className="px-5 pt-8 pb-6">
            <div className="flex items-center gap-3">
              <div
                ...
                style={{ background: "linear-gradient(135deg, #0078d4, #60cdff)" }}
              >
                A
              </div>
              <div>
          ...
          <nav className="flex-1 px-3 py-2 space-y-0.5">
            {navItems.map((item) => (
              <NavLink
                ...
                to={item.to}
                className={({ isActive }) =>
                  `flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all duration-150 ${isActive ? "font-medium" : ""}`
                }
                style={({ isActive }) => ({
