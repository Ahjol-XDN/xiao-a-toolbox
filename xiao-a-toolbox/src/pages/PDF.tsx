...
export default function PDFPage() {
  ...
  const { pdfMerge, pdfSplit, cancelConvert, notify, getConfig, addHistory } = useIPC();
    ...
    try {
      if (mode === "merge") {
        const savePath = outputDir
          ? `${outputDir}\\merged.pdf`
          : await window.electronAPI.saveFile({ filters: [{ name: "PDF", extensions: ["pdf"] }], defaultPath: "merged.pdf" });
        if (!savePath) { setRunning(false); return; }
        await pdfMerge({ inputPaths: files.map((f) => f.path), outputPath: savePath });
        addHistory({ input: files.map((f) => f.name).join(","), output: savePath, format: "pdf", mode: "pdfMerge", status: "success" });
        notify("合并完成", "PDF 已成功合并");
      } else {
        const saveDir = outputDir
          ? outputDir
          : (await window.electronAPI.saveFile({ defaultPath: "split_output" }));
        if (!saveDir) { setRunning(false); return; }
        const actualDir = saveDir.replace(/\\[^\\]*$/, "");
        const ranges = splitRanges.split(",").map((r) => {
          const [s, e] = r.split("-").map(Number);
          return { start: s, end: e };
        });
        await pdfSplit({ inputPath: files[0].path, outputDir: actualDir, ranges });
        addHistory({ input: files[0].path, output: actualDir, format: "pdf", mode: "pdfSplit", status: "success" });
        notify("拆分完成", "PDF 已成功拆分");
      }
    ...
    <div>
      ...
      <DropZone onFilesDrop={handleDrop}>
        <div onClick={handleClick}
          ...
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
            {mode === "merge" ? "拖拽多个 PDF（按顺序合并）" : "拖拽一个 PDF 文件"}，或点击选择
          </p>
        </div>
      </DropZone>
      ...
      {mode === "split" && (
        <div className="mt-4 rounded-xl border p-5" style={{ background: "var(--bg-card)", borderColor: "var(--border-subtle)", boxShadow: "var(--shadow-sm)" }}>
          ...
          <input type="text" value={splitRanges} onChange={(e) => setSplitRanges(e.target.value)} className="w-full px-3 py-2 rounded-lg border text-sm font-mono outline-none" style={is} />
