...
export default function BatchPage() {
  ...
  const { batchConvert, startProgress, stopProgress, cancelConvert } = useIPC();
  ...
  const handleClick = async () => {
    const paths = await window.electronAPI.openFile([{ name: "所有支持格式", extensions: ["mp4","mkv","avi","mov","webm","flv","mp3","wav","aac","flac","ogg","png","jpg","jpeg","webp","bmp","docx","pdf","md","txt"] }]);
    if (paths?.length) addFiles(paths);
  };
  const selectOutputDir = async () => {
    const dir = await window.electronAPI.saveFile({ defaultPath: "output" });
    if (dir) setOutputDir(dir.replace(/\\[^\\]*$/, ""));
  };
  const doConvert = async () => {
    ...
    const tasks = files.map((f) => ({ inputPath: f.path, outputPath: outputDir + "\\" + f.name.replace(/\.[^.]+$/, "." + format), format }));
    try { const res = await batchConvert(tasks); setResults(res); setProgress((p: any) => ({ ...p, percent: 100 })); }
      ...
      <div className="mt-4 rounded-xl border p-5 space-y-4" style={{ background: "var(--bg-card)", borderColor: "var(--border-subtle)", boxShadow: "var(--shadow-sm)" }}>
        ...
        <div>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={selectOutputDir} className="px-4 py-2 rounded-lg text-sm font-medium transition-colors"
              style={{ background: "var(--bg-secondary)", color: "var(--text-secondary)" }}>选择目录</button>
          </div>
        </div>
      </div>
      {results.length > 0 && (
